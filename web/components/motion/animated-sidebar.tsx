...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "mb-1 h-7 overflow-hidden px-2 text-xs tracking-normal font-medium font-sans text-muted-foreground transition-opacity",
...
export interface AnimatedSidebarMenuSubButtonProps {
  ...
  className?: string;
  /**
   * What an `href` renders as. Defaults to a plain anchor; a router-aware link
   * (`motion.create(Link)`) keeps navigation client-side.
   */
...
export function AnimatedSidebarMenuSubButton({
  ...
  const LinkTag = linkComponent ?? motion.a;
